...
$(document).ready(function() {
    ...
    // Inisialisasi DataTable SEKALI SAJA, pakai ajax bawaan
    const table = $('#defectTable').DataTable({
        ...
        processing: true,
        ajax: {
            url: 'internaldefectController.php',
            data: function(d) {
            ...
            dataSrc: function(json) {
                if (!json.success) {
                    Swal.fire({ icon: 'error', title: 'Error', text: json.message || 'Gagal memuat data' });
                    return [];
                }
                return json.data || [];
            },
            error: function(xhr, status, error) {
                Swal.fire({ icon: 'error', title: 'Error', text: 'Terjadi kesalahan saat memuat data: ' + error });
            }
        },
        columns: [
            { data: null, render: (d, t, r, meta) => meta.row + 1 },
            { data: 'Lot', render: d => `<span class="badge bg-info">${d || '-'}</span>` },
            { data: 'Skema', defaultContent: '-' },
            { data: 'PartName', defaultContent: '-' },
            { data: 'TERMINALA', render: d => `<span class="badge bg-primary">${d || '-'}</span>` },
            { data: 'TERMINALB', render: d => `<span class="badge bg-success">${d || '-'}</span>` },
            { data: 'JenisDefect', render: d => `<span class="badge bg-danger">${d || '-'}</span>` },
            { data: 'Qty', className: 'text-end fw-bold', render: d => parseInt(d || 0).toLocaleString() },
            { data: 'Tanggal', defaultContent: '-' },
            { data: 'Jam', defaultContent: '-' },
            { data: 'Shift', render: function(d) {
                let color = 'secondary';
            ...
            { data: 'KodeMesin', render: d => `<span class="badge bg-secondary">${d || '-'}</span>` },
            { data: 'Section', render: function(d) {
                let color = 'secondary';
                if (d === 'CNC') color = 'info';
        ...
        dom: 'Bfrtip',
        buttons: [
            { extend: 'excel', text: '<i class="fas fa-file-excel"></i> Excel', className: 'btn btn-success btn-sm', title: 'Data Defect Internal' },
            { extend: 'pdf', text: '<i class="fas fa-file-pdf"></i> PDF', className: 'btn btn-danger btn-sm', title: 'Data Defect Internal' },
    ...
    $('#applyFilter').on('click', function() {
        if (!filters.date_start || !filters.date_end) {
            Swal.fire({ icon: 'warning', title: 'Peringatan', text: 'Silakan pilih range tanggal terlebih dahulu' });
            return;
        }
        loadData();
        Swal.fire({ icon: 'success', title: 'Filter Diterapkan', timer: 1500, showConfirmButton: false });
    });
    ...
    $('#resetFilter').on('click', function() {
        ...
        filters.section = 'all';
        loadData();
        Swal.fire({ icon: 'info', title: 'Filter Direset', timer: 1500, showConfirmButton: false });
    });
