...
//     $sql = "SELECT DISTINCT PartNameFG AS partno FROM TRRPHMESIN WHERE ItemFGS IS NOT NULL AND PartNameFG != '' AND PartNameFG != 'Coba-JanganDipakai' ORDER BY PartNameFG ASC";
...
{
    $sql = "
        SELECT DISTINCT tipartnumber AS partno
        FROM dbInventory2.dbo.tblItemSimbol
        WHERE tiid LIKE '%FGS%'
          AND tisite = 'PLANT-1'
          AND tipartnumber != ''
        ORDER BY tipartnumber ASC
    ";
...
// WHERE t.PartNameFG LIKE ?";
    ...
    // Query Production
    $sql = "SELECT TOP (1) 
            b.tshnamacust as namacustomer
        FROM dbInventory2.dbo.tblSOd a
        LEFT JOIN dbInventory2.dbo.tblSOh b 
            ON b.tshno = a.tshid
        WHERE a.tsdtiid = (
