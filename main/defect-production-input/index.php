...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function fetchDefects(section) {
            ...
            rows.forEach(function(row) {
                ...
                fetch('inputdefectProdController.php?action=listDefect&section=' + encodeURIComponent(section))
        ...
        function loadFromLocalStorage() {
            ...
            if (savedSection) {
                ...
                // Fetch defects setelah section di-set
                if (savedSection) {
                    setTimeout(() => {
                        fetchDefects(savedSection);
                    }, 100);
                }
            }
        ...
        <td>
            <select class="form-select" name="defect[]" required>
                ...
                <!-- Options akan diisi via fetchDefects -->
            </select>
        </td>
        <td>
    ...
    `;
            ...
            // Fetch defect untuk baris baru jika section sudah dipilih
            if (sectionSelect.value) {
                // Delay sedikit untuk memastikan select sudah terpasang
                setTimeout(() => {
                    const defectSelect = tr.querySelector('select[name="defect[]"]');
                    fetchDefectsForRow(sectionSelect.value, defectSelect);
                }, 50);
            }
        ...
        // Fungsi tambahan untuk fetch defect per row
        function fetchDefectsForRow(section, defectSelect) {
            if (!section || !defectSelect) return;
