...
function insertDefect($connection)
{
    ...
    // Filter defects yang tidak kosong DAN ganti koma dengan garis miring
    $valid_defects = [];
    $replaced_defects = []; // Untuk mencatat defect yang mengalami penggantian

    foreach ($nama_defects as $defect) {
        $defect_trimmed = trim($defect);
        if (!empty($defect_trimmed)) {
            // GANTI KOMA (,) MENJADI GARIS MIRING (/)
            $original_defect = $defect_trimmed;
            $defect_modified = str_replace(',', '/', $defect_trimmed);

            // Catat jika ada perubahan
            if ($original_defect !== $defect_modified) {
                $replaced_defects[] = [
                    'original' => $original_defect,
                    'modified' => $defect_modified
                ];
            }

            $valid_defects[] = $defect_modified;
        }
    }
    ...
    if ($success_count > 0) {
        ...
        // Info perubahan koma ke garis miring
        if (!empty($replaced_defects)) {
            $message .= ". Perubahan koma(,) ke garis miring(/) pada: ";
            $changes = [];
            foreach ($replaced_defects as $change) {
                $changes[] = "'{$change['original']}' → '{$change['modified']}'";
            }
        ...
        echo json_encode([
            ...
            'replaced_defects' => $replaced_defects // Kirim info perubahan ke FE
        ]);
    } else {
...
function updateDefect($connection)
{
    ...
    $nama_defect_original = trim($nama_defect_raw);

    // GANTI KOMA DENGAN SLASH UNTUK MASTER
    $nama_defect_baru = str_replace(',', '/', $nama_defect_original);
    $is_replaced = ($nama_defect_original !== $nama_defect_baru);
    ...
    $sql_old = "SELECT nama_section, nama_defect FROM defect_table WHERE id = ?";
    ...
    $old_section = trim($old_data['nama_section']);
    $old_defect = trim($old_data['nama_defect']);

    // 🔥 STEP 1: AMBIL SEMUA DATA DEFECT DARI MASTER (untuk referensi parsing)
    $sql_all_master = "SELECT id, nama_section, nama_defect FROM defect_table";
    $stmt_master = sqlsrv_query($connection, $sql_all_master);
    $master_defects = [];
    while ($row = sqlsrv_fetch_array($stmt_master, SQLSRV_FETCH_ASSOC)) {
        $master_defects[] = [
            'id' => $row['id'],
            'nama_section' => trim($row['nama_section']),
            'nama_defect' => trim($row['nama_defect'])
        ];
    }

    // 🔥 STEP 2: CEK APAKAH NAMA DEFECT BARU SUDAH ADA DI MASTER (CASE INSENSITIVE)
    $defect_exists = false;
    $existing_defect_id = null;
    $existing_defect_name = null;

    foreach ($master_defects as $master) {
        if (strcasecmp($master['nama_defect'], $nama_defect_baru) === 0 && $master['id'] != $id) {
            $defect_exists = true;
            $existing_defect_id = $master['id'];
            $existing_defect_name = $master['nama_defect'];
            break;
        }
    }
    ...
    if ($defect_exists) {
        ...
        echo json_encode([
            'status' => 'error',
            'message' => "Defect '$nama_defect_baru' sudah ada di master dengan ID: $existing_defect_id"
        ]);
        exit;
    }
    ...
    $existing_section_name = null;

    foreach ($master_defects as $master) {
        if (strcasecmp($master['nama_section'], $nama_section) === 0 && $master['id'] != $id) {
            $section_exists = true;
            $existing_section_name = $master['nama_section'];
            break;
        }
    }

    if ($section_exists) {
        $nama_section = $existing_section_name;
    }

    // 🔥 STEP 4: AMBIL SEMUA TRANSAKSI YANG MENGANDUNG OLD_DEFECT
    // Buat berbagai kemungkinan format old_defect
    $old_defect_variants = [
        $old_defect,                           // versi asli (mungkin sudah pakai slash)
        str_replace('/', ',', $old_defect),    // versi dengan koma
        str_replace('/', ' ', $old_defect),    // versi dengan spasi
        trim($old_defect),                     // trim
    ];
    $old_defect_variants = array_unique($old_defect_variants);

    // Query untuk mencari transaksi
    $sql_get_claims = "SELECT id, nama_defect FROM report_claim_defect WHERE ";
    $conditions = [];
    $params = [];

    foreach ($old_defect_variants as $variant) {
        $conditions[] = "nama_defect LIKE ?";
        $params[] = '%' . str_replace(['%', '_'], ['\\%', '\\_'], $variant) . '%';
    }
    $sql_get_claims .= implode(' OR ', $conditions);

    $stmt_claims = sqlsrv_query($connection, $sql_get_claims, $params);

    if (!$stmt_claims) {
        http_response_code(500);
        echo json_encode(['status' => 'error', 'message' => 'Gagal mengambil data transaksi']);
        exit;
    }

    // 🔥 STEP 5: PROSES UPDATE TRANSAKSI DULU
    $updated_count = 0;
    $updated_ids = [];
    $detail_changes = [];
    ...
    while ($row = sqlsrv_fetch_array($stmt_claims, SQLSRV_FETCH_ASSOC)) {
        $claim_id = $row['id'];
        $original_value = $row['nama_defect'];

        // 🔥 LOGIKA PARSING CERDAS BERDASARKAN MASTER
        // Kita coba split berdasarkan koma, tapi kita harus tahu mana yang merupakan 1 defect utuh

        // Method: Cari substring yang match dengan master defects
        $new_value = $original_value;

        foreach ($old_defect_variants as $variant) {
            // Cari posisi variant dalam string
            $pos = stripos($new_value, $variant);
            if ($pos !== false) {
                // Ganti dengan nama_defect_baru
                $new_value = substr_replace($new_value, $nama_defect_baru, $pos, strlen($variant));
            }
        }

        // 🔥 CEK APAKAH ADA PERUBAHAN
        if ($new_value !== $original_value) {
            $sql_update = "UPDATE report_claim_defect SET nama_defect = ? WHERE id = ?";
            $stmt_update = sqlsrv_query($connection, $sql_update, [$new_value, $claim_id]);
    ...
    // 🔥 STEP 6: UPDATE MASTER (TERAKHIR)
    sqlsrv_begin_transaction($connection);

    try {
        $sql_update_master = "UPDATE defect_table SET nama_section = ?, nama_defect = ? WHERE id = ?";
        $stmt_master_update = sqlsrv_prepare($connection, $sql_update_master, [$nama_section, $nama_defect_baru, $id]);

        if (!$stmt_master_update || !sqlsrv_execute($stmt_master_update)) {
        ...
        $message = "Defect berhasil diupdate";
        if ($is_replaced) {
            $message .= " (koma diganti slash: '$nama_defect_original' → '$nama_defect_baru')";
        }
        $message .= " | Transaksi terupdate: $updated_count baris";
