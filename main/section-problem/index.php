...
<script>
    ...
    $(document).ready(function() {
        ...
        // Update info text dengan informasi konversi koma
        $('#infoText').html(`
        <i class="ti ti-info-circle"></i> 
        Anda bisa menambahkan multiple defect sekaligus. 
        <strong class="text-warning">Koma (,) akan otomatis diganti menjadi garis miring (/)</strong>
    `);
    });
    ...
    function checkAndConfirmCommaReplacement() {
        let defects = [];
        $('input[name="nama_defect[]"]').each(function() {
    ...
    function showCommaReplacementConfirmation(replacedItems, onConfirm) {
        let itemsHtml = '<div style="max-height: 300px; overflow-y: auto;">';
        replacedItems.forEach(item => {
            itemsHtml += `
            <div class="alert alert-warning mb-2 p-2">
                <i class="ti ti-alert-triangle"></i>
                <strong>Sebelum:</strong> ${escapeHtml(item.original)}<br>
                <strong>Sesudah:</strong> ${escapeHtml(item.modified)}
            </div>
        `;
        });
        itemsHtml += '</div>';
        ...
        Swal.fire({
            title: 'Konfirmasi Penggantian Koma',
            html: `
            <div class="text-start">
                <p class="mb-2">Sistem akan mengganti <strong>koma (,)</strong> menjadi <strong>garis miring (/)</strong> pada defect berikut:</p>
                ${itemsHtml}
                <hr>
                <p class="text-muted small mb-0">Apakah Anda ingin melanjutkan?</p>
            </div>
        `,
            icon: 'warning',
            ...
            cancelButtonText: 'Batal, Edit Manual',
            preConfirm: () => {
                return replacedItems;
            }
        }).then((result) => {
            if (result.isConfirmed && onConfirm) {
                onConfirm(replacedItems);
            }
        });
    }
    ...
    function showAddDefectModal() {
        ...
        // Reset input defect pertama
        $('input[name="nama_defect[]"]').first().val('').focus();
    ...
    $('#defectForm').on('submit', function(e) {
        ...
        // CEK KEBUTUHAN KONFIRMASI PENGGANTIAN KOMA
        let action = $('#defectAction').val();

        // Hanya tampilkan konfirmasi untuk insert, tidak untuk update
        if (action === 'insert') {
            let commaCheck = checkAndConfirmCommaReplacement();

            if (commaCheck.needsConfirmation) {
                // Tampilkan konfirmasi
                showCommaReplacementConfirmation(commaCheck.replacedItems, function(replacedItems) {
                    // Set nilai final nama_section ke hidden input
                    $('#finalNamaSection').val(namaSection);
                    submitDefectForm();
                });
            } else {
                // Tidak ada koma, langsung submit
                $('#finalNamaSection').val(namaSection);
                submitDefectForm();
            }
        } else {
            // Untuk update, langsung submit tanpa konfirmasi (karena BE sudah handle)
            $('#finalNamaSection').val(namaSection);
            submitDefectForm();
        }
    });

    // Fungsi terpisah untuk submit form
    function submitDefectForm() {
        ...
        let formData = new FormData($('#defectForm')[0]);
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.status === 'success') {
                    ...
                    // Tampilkan informasi tambahan jika ada penggantian koma
                    if (response.replaced_defects && response.replaced_defects.length > 0) {
                        message += '\n\n' + response.replaced_defects.map(r =>
