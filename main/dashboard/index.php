...
<div class="container-fluid">
    ...
    <!-- Stats Hari Ini: NG, Total Defect, OK -->
    <div class="row mt-4">
        <div class="col-12 mb-4">
            <div class="card shadow-sm">
                <div class="card-header bg-transparent">
                    <h5 class="card-title mb-0">
                        <i class="ti ti-calendar-stats me-2"></i>Statistik Hari Ini
                    </h5>
                </div>
                    ...
                    <div class="row g-0">
                        ...
                        <div class="col-md-4 border-end">
                            <div class="p-4 text-center">
                                <div class="bg-light-danger rounded-2 p-2 d-inline-block mb-3">
                                    <i class="ti ti-circle-x fs-4 text-danger"></i>
                                </div>
                                <p class="text-dark mb-1 fw-semibold">Total NG Hari Ini</p>
                                <h3 class="mb-0 fw-bold text-danger" id="total-ng-today">0</h3>
                            </div>
                        </div>

                        <!-- Total Defect Hari Ini (Bisa Diklik) -->
                        <div class="col-md-4 border-end">
                            <div class="p-4 text-center" id="total-defect-today-card" style="cursor: pointer;">
                                <div class="bg-light-primary rounded-2 p-2 d-inline-block mb-3">
                                    <i class="ti ti-bug fs-4 text-primary"></i>
                                </div>
                                <p class="text-dark mb-1 fw-semibold">
                                    Total Defect Hari Ini
                                    <i class="ti ti-click text-info ms-1" style="font-size: 12px;"></i>
                                </p>
                                <h3 class="mb-0 fw-bold text-primary" id="total-defect-today">0</h3>
                                <small class="text-muted mt-2 d-block">
                                    <i class="ti ti-info-circle"></i> Klik untuk detail
                                </small>
                            </div>
                        </div>
                        ...
                        <div class="col-md-4">
                            <div class="p-4 text-center">
                                <div class="bg-light-success rounded-2 p-2 d-inline-block mb-3">
                                    <i class="ti ti-circle-check fs-4 text-success"></i>
                                </div>
                                <p class="text-dark mb-1 fw-semibold">Total OK Hari Ini</p>
                                <h3 class="mb-0 fw-bold text-success" id="total-ok-today">0</h3>
                            </div>
                        </div>
                    </div>
                </div>
...
<!-- Modal Detail Defect Hari Ini -->
<div class="modal fade" id="modalDetailDefect" tabindex="-1" aria-labelledby="modalDetailDefectLabel"
    aria-hidden="true">
    <div class="modal-dialog modal-xl modal-dialog-scrollable">
        <div class="modal-content">
            <div class="modal-header bg-primary text-white">
                <h5 class="modal-title fw-semibold" id="modalDetailDefectLabel">
                    <i class="ti ti-bug me-2"></i>Detail Defect Hari Ini
                </h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"
                    aria-label="Close"></button>
            </div>
            <div class="modal-body p-3">
                <!-- Loading indicator di dalam modal -->
                <div id="loading-modal" class="text-center py-5" style="display: none;">
                    <div class="spinner-border text-primary" role="status">
                        <span class="visually-hidden">Loading...</span>
                    </div>
                    <p class="mt-3 text-muted">Memuat data defect hari ini...</p>
                </div>

                <!-- Tabel DataTables -->
                <div class="table-responsive">
                    <table id="table-today-defect" class="table table-bordered table-hover w-100">
                        <thead class="table-light">
                            <tr>
                                ...
                                <th width="10%">Qty</th>
                                <th width="12%">Tanggal Ditemukan</th>
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary" id="btn-refresh-modal">
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const defectTodayCard = document.getElementById('total-defect-today-card');
        if (defectTodayCard) {
            defectTodayCard.addEventListener('click', function () {
                openDefectDetailModal();
            });
        }

        // Event refresh button di modal
    ...
    function openDefectDetailModal() {
        const modal = new bootstrap.Modal(document.getElementById('modalDetailDefect'));
        modal.show();
        loadTodayDefectData();
    }

    // Function untuk load data ke DataTable
    function loadTodayDefectData() {
        const loadingModal = document.getElementById('loading-modal');
        const tableElement = document.getElementById('table-today-defect');

        // Tampilkan loading, sembunyikan tabel sementara
        if (loadingModal) loadingModal.style.display = 'block';
        if (tableElement) tableElement.style.display = 'none';

        // Jika DataTable sudah ada, destroy dulu
        if (todayDefectTable) {
            todayDefectTable.destroy();
            todayDefectTable = null;
        }

        // Kosongkan tbody
        const tbody = document.querySelector('#table-today-defect tbody');
        if (tbody) {
            tbody.innerHTML = '<tr><td colspan="8" class="text-center">Memuat data...</td></tr>';
        }

        // Panggil API dashboard untuk getTodayDefects
        fetch(API.dashboard + '?action=getTodayDefects')
            .then(response => {
                if (!response.ok) {
            ...
            .then(result => {
                ...
                if (result.status === 'success' && result.data && result.data.length > 0) {
                    ...
                    if (tbody) {
                        tbody.innerHTML = '<tr><td colspan="8" class="text-center text-muted py-5"><i class="ti ti-database-off fs-4"></i><br>Tidak ada data defect untuk hari ini</td></tr>';
                    }
                    console.log('Tidak ada data defect hari ini');
                }
            })
            .catch(error => {
                console.error('Error loading today defects:', error);
                if (loadingModal) loadingModal.style.display = 'none';
                if (tableElement) tableElement.style.display = 'table';

                const tbody = document.querySelector('#table-today-defect tbody');
                if (tbody) {
                    tbody.innerHTML = '<tr><td colspan="8" class="text-center text-danger py-5"><i class="ti ti-alert-circle fs-4"></i><br>Gagal memuat data: ' + error.message + '</td></tr>';
                }
            });
    }
    ...
    function initializeDataTable(data) {
        ...
        todayDefectTable = $('#table-today-defect').DataTable({
            data: tableData,
            columns: [
                { title: "No", width: "5%" },
                { title: "Section", width: "12%" },
                { title: "Defect", width: "15%" },
                { title: "Lot No", width: "15%" },
                { title: "Part No", width: "15%" },
                { title: "Qty", width: "10%", className: "text-center" },
                { title: "Tanggal Ditemukan", width: "12%" },
                { title: "Customer", width: "16%" }
            ],
            language: {
            ...
            buttons: [
                {
                    ...
                    title: 'Defect_Hari_Ini_' + new Date().toISOString().slice(0, 10)
                },
                {
                    extend: 'pdf',
                ...
                },
                {
                    ...
                    title: 'Laporan Defect Hari Ini'
                }
            ]
        });

        console.log('DataTable berhasil diinisialisasi dengan', data.length, 'data');
    }

    // Function untuk refresh data
    function refreshDefectTable() {
        if (todayDefectTable) {
            todayDefectTable.destroy();
            todayDefectTable = null;
        }
        ...
        fetch(API.dashboard + '?action=getDashboardStats')
            ...
            .then(result => {
                ...
                if (result.status === 'success') {
                    ...
                    document.getElementById('total-ng-today').textContent = result.data.total_hari_ini_ng || 0;
                    document.getElementById('total-defect-today').textContent = result.data.total_hari_ini || 0;
                    document.getElementById('total-ok-today').textContent = result.data.total_hari_ini_ok || 0;
                    ...
                    document.getElementById('total-ng-today').textContent = '0';
                    document.getElementById('total-defect-today').textContent = '0';
                    document.getElementById('total-ok-today').textContent = '0';
                }
            })
            .catch(error => {
                // Sembunyikan loading
...
<style>
    ...
    /* Style untuk card Total Defect Hari Ini yang bisa diklik */
    #total-defect-today-card {
        transition: all 0.3s ease;
    }

    #total-defect-today-card:hover {
        background-color: rgba(13, 110, 253, 0.05);
        transform: translateY(-3px);
    }

    #total-defect-today-card:active {
        transform: translateY(0);
    }

    /* Indikator bahwa element bisa diklik */
    #total-defect-today-card .ti-click {
        opacity: 0.7;
        transition: opacity 0.3s ease;
    }

    #total-defect-today-card:hover .ti-click {
        opacity: 1;
    }
